...
YOUTUBE_LANG_PRIORITY = [
    "zh-Hans", "zh-Hant", "zh-CN", "zh-TW", "zh-HK", "zh", "zh-Hans-en", "zh-Hant-en", "en"
]

YOUTUBE_ENGLISH_PRIORITY = [
...
class YouTubeService(SubtitleService):
    ...
    def _select_lang(self, available: List[str], language: str = "zh") -> Optional[str]:
        if language == "en":
            for lang in YOUTUBE_ENGLISH_PRIORITY:
    ...
    async def download_subtitle(
        ...
        try:
            ...
            if available:
                lang = self._select_lang(available, language)
                if not lang:
                    return {"error": "无法选择字幕"}
